...
import { ChevronDown, Calendar, Plus, Check, Upload, X, CheckCircle, AlertCircle } from 'lucide-react';
...
const LaporMelaluiWebsite = () => {
  ...
  const [reports, setReports] = useState([
    {
      ...
      description: "Jalan utama di Malioboro mengalami kerusakan parah dengan banyak lubang besar yang membahayakan pengendara. Kondisi ini sudah berlangsung selama 2 minggu dan menyebabkan kemacetan serta kecelakaan ringan. Warga mengeluh karena akses menuju pusat kota terganggu.",
      date: "8 Agustus 2025",
    ...
    },
    {
      ...
      description: "Sungai Code mengalami pencemaran berat akibat pembuangan limbah industri. Air sungai berubah warna menjadi kehitaman dan berbau busuk. Ikan-ikan mati dan warga sekitar mengeluh karena air sumur mereka ikut tercemar. Kondisi ini sangat mengkhawatirkan kesehatan masyarakat.",
      date: "9 Agustus 2025",
    ...
    },
    {
      ...
      description: "Terjadi praktik pungutan liar oleh oknum yang mengaku sebagai petugas di Terminal Giwangan. Mereka memungut biaya tambahan kepada penumpang dan sopir angkutan umum. Banyak korban yang merasa dirugikan namun takut melapor karena ancaman. Praktik ini sudah berlangsung berbulan-bulan.",
    ...
    setReports(prevReports =>
      prevReports.map(report => {
        if (report.id === reportId) {
          ...
          return { ...report, status: report.status === 'added' ? 'pending' : 'added' };
        } else {
  ...
  const handleSendReports = () => {
    const hasSelectedReport = reports.some(report => report.status === 'added');

    if (hasSelectedReport) {
      setNotification({
        type: 'success',
        title: 'Laporan Berhasil Dikirim!',
        message: 'Laporan Anda telah berhasil dikirim bersama dengan rekomendasi yang dipilih. Tim terkait akan segera menindaklanjuti laporan Anda.'
      });
    } else {
      setNotification({
        type: 'success',
        title: 'Laporan Berhasil Dikirim!',
        message: 'Laporan Anda telah berhasil dikirim. Tim terkait akan segera menindaklanjuti laporan Anda.'
      });
    }
  ...
  const handleFinalSend = () => {
    setNotification({
      ...
      message: 'Semua laporan yang dipilih telah berhasil dikirim. Terima kasih atas partisipasi Anda dalam meningkatkan keamanan dan kenyamanan masyarakat.'
    });
      ...
      <div>
        ...
        <div className="relative">
          <select
            ...
            onChange={(e) => handleInputChange('instansiTujuan', e.target.value)}
            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 appearance-none"
          >
      ...
      <div className="space-y-3">
        <div className="flex items-center space-x-3">
          <input
            ...
            onChange={(e) => handleInputChange('tampilkanPublik', e.target.checked)}
        ...
        <div className="flex items-center space-x-3">
          <input
            ...
            onChange={(e) => handleInputChange('tampilkanAnonim', e.target.checked)}
            className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
          />
  ...
  const renderStep2 = () => {
    ...
    return (
      <div className="space-y-6">
        {reports.map((report) => (
          <div key={report.id} className={`bg-white border-l-4 rounded-lg p-6 shadow-sm ${report.status === 'added' ? 'border-green-400 bg-green-50' : 'border-yellow-400'
            }`}>
            ...
            <div className="space-y-3 text-sm text-gray-600">
              ...
              <div>
                <span className="font-medium text-gray-700">Tanggal dan Lokasi</span>
                <p className="mt-1">{report.date} • {report.location}</p>
              </div>

              <div>
                <span className="font-medium text-gray-700">Instansi yang dituju</span>
        ...
        <div className="flex space-x-4">
          ...
          >
            {hasSelectedReport ? 'Kirim' : 'Lewati dan Kirim'}
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-bl from-[#00294A] to-[#336F9F] pt-36 px-12">
      ...
      {notification && (
        <div className="fixed top-4 right-4 z-50 max-w-md w-full">
          <div className={`bg-white rounded-lg shadow-lg border-l-4 p-4 ${notification.type === 'success' ? 'border-green-500' : 'border-red-500'
            }`}>
        ...
        <div className="bg-white rounded-xl shadow-xl p-8">
          ...
          <div className="text-center mb-8">
            ...
            <div className="flex items-center justify-center space-x-8 mb-8">
              <div className="flex items-center space-x-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium ${currentStep >= 1 ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-600'
                  }`}>
                  1
                </div>
                <span className="text-sm text-gray-600">Pengisian Formulir</span>
              </div>

              <div className="flex-1 h-px bg-gray-300"></div>

              <div className="flex items-center space-x-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium ${currentStep >= 2 ? 'bg-yellow-500 text-white' : 'bg-gray-200 text-gray-600'
                  }`}>
                  2
                </div>
                <span className="text-sm text-gray-600">Rekomendasi Laporan</span>
              </div>
            </div>
          </div>
